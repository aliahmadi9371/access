/*******************************************************************************
* File Name: RS485_Sensor_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void RS485_Sensor_IntClock_Start(void) 
{
    /* Set the bit to enable the clock. */
    RS485_Sensor_IntClock_CLKEN |= RS485_Sensor_IntClock_CLKEN_MASK;
	RS485_Sensor_IntClock_CLKSTBY |= RS485_Sensor_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void RS485_Sensor_IntClock_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    RS485_Sensor_IntClock_CLKEN &= (uint8)(~RS485_Sensor_IntClock_CLKEN_MASK);
	RS485_Sensor_IntClock_CLKSTBY &= (uint8)(~RS485_Sensor_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void RS485_Sensor_IntClock_StopBlock(void) 
{
    if ((RS485_Sensor_IntClock_CLKEN & RS485_Sensor_IntClock_CLKEN_MASK) != 0u)
    {
...
#if defined(RS485_Sensor_IntClock__CFG3)
        CLK_DIST_AMASK = RS485_Sensor_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = RS485_Sensor_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* RS485_Sensor_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(RS485_Sensor_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(RS485_Sensor_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void RS485_Sensor_IntClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        RS485_Sensor_IntClock_CLKSTBY &= (uint8)(~RS485_Sensor_IntClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void RS485_Sensor_IntClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = RS485_Sensor_IntClock_GetSourceRegister();
    uint16 oldDivider = RS485_Sensor_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = RS485_Sensor_IntClock_CLKEN & RS485_Sensor_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(RS485_Sensor_IntClock_DIV_PTR, clkDivider);
                RS485_Sensor_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                RS485_Sensor_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(RS485_Sensor_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((RS485_Sensor_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(RS485_Sensor_IntClock_DIV_PTR, clkDivider);
				RS485_Sensor_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 RS485_Sensor_IntClock_GetDividerRegister(void) 
{
    return CY_GET_REG16(RS485_Sensor_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void RS485_Sensor_IntClock_SetModeRegister(uint8 modeBitMask) 
{
    RS485_Sensor_IntClock_MOD_SRC |= modeBitMask & (uint8)RS485_Sensor_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void RS485_Sensor_IntClock_ClearModeRegister(uint8 modeBitMask) 
{
    RS485_Sensor_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(RS485_Sensor_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 RS485_Sensor_IntClock_GetModeRegister(void) 
{
    return RS485_Sensor_IntClock_MOD_SRC & (uint8)(RS485_Sensor_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void RS485_Sensor_IntClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = RS485_Sensor_IntClock_GetDividerRegister();
    uint8 oldSrc = RS485_Sensor_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        RS485_Sensor_IntClock_MOD_SRC |= CYCLK_SSS;
        RS485_Sensor_IntClock_MOD_SRC =
            (RS485_Sensor_IntClock_MOD_SRC & (uint8)(~RS485_Sensor_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 RS485_Sensor_IntClock_GetSourceRegister(void) 
{
    return RS485_Sensor_IntClock_MOD_SRC & RS485_Sensor_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void RS485_Sensor_IntClock_SetPhaseRegister(uint8 clkPhase) 
{
    RS485_Sensor_IntClock_PHASE = clkPhase & RS485_Sensor_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: RS485_Sensor_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 RS485_Sensor_IntClock_GetPhaseRegister(void) 
{
    return RS485_Sensor_IntClock_PHASE & RS485_Sensor_IntClock_PHASE_MASK;
}
