/*******************************************************************************
* File Name: Clock_Timer_Pin_Port2_W2.c
* Version 2.20
*
*  Description:
...
#include "Clock_Timer_Pin_Port2_W2.h"
...
/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_Start
********************************************************************************
*
* Summary:
...
void Clock_Timer_Pin_Port2_W2_Start(void) 
{
    ...
    Clock_Timer_Pin_Port2_W2_CLKEN |= Clock_Timer_Pin_Port2_W2_CLKEN_MASK;
	Clock_Timer_Pin_Port2_W2_CLKSTBY |= Clock_Timer_Pin_Port2_W2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_Stop
********************************************************************************
*
* Summary:
...
void Clock_Timer_Pin_Port2_W2_Stop(void) 
{
    ...
    Clock_Timer_Pin_Port2_W2_CLKEN &= (uint8)(~Clock_Timer_Pin_Port2_W2_CLKEN_MASK);
	Clock_Timer_Pin_Port2_W2_CLKSTBY &= (uint8)(~Clock_Timer_Pin_Port2_W2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Timer_Pin_Port2_W2_CLKEN & Clock_Timer_Pin_Port2_W2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Timer_Pin_Port2_W2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Timer_Pin_Port2_W2_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Timer_Pin_Port2_W2__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Timer_Pin_Port2_W2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Timer_Pin_Port2_W2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Timer_Pin_Port2_W2_GetSourceRegister();
    uint16 oldDivider = Clock_Timer_Pin_Port2_W2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Timer_Pin_Port2_W2_CLKEN & Clock_Timer_Pin_Port2_W2_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_Timer_Pin_Port2_W2_DIV_PTR, clkDivider);
                Clock_Timer_Pin_Port2_W2_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_Timer_Pin_Port2_W2_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_Timer_Pin_Port2_W2_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Timer_Pin_Port2_W2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Clock_Timer_Pin_Port2_W2_DIV_PTR, clkDivider);
				Clock_Timer_Pin_Port2_W2_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Timer_Pin_Port2_W2_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Timer_Pin_Port2_W2_MOD_SRC |= modeBitMask & (uint8)Clock_Timer_Pin_Port2_W2_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Timer_Pin_Port2_W2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Timer_Pin_Port2_W2_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Timer_Pin_Port2_W2_MOD_SRC & (uint8)(Clock_Timer_Pin_Port2_W2_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Timer_Pin_Port2_W2_GetDividerRegister();
    uint8 oldSrc = Clock_Timer_Pin_Port2_W2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Timer_Pin_Port2_W2_MOD_SRC |= CYCLK_SSS;
        Clock_Timer_Pin_Port2_W2_MOD_SRC =
            (Clock_Timer_Pin_Port2_W2_MOD_SRC & (uint8)(~Clock_Timer_Pin_Port2_W2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Timer_Pin_Port2_W2_MOD_SRC & Clock_Timer_Pin_Port2_W2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Timer_Pin_Port2_W2_PHASE = clkPhase & Clock_Timer_Pin_Port2_W2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Timer_Pin_Port2_W2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Timer_Pin_Port2_W2_PHASE & Clock_Timer_Pin_Port2_W2_PHASE_MASK;
}
