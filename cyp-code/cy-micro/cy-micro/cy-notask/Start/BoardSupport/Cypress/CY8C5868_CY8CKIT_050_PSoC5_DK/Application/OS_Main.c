...
void Port1_Job(uint8 intr)
{
    ...
    uint8 Port1 = 0;
    Port1 = intr;
    check_port_1(Port1, &Port1_W0, &Port1_W1, &Port1_In);
...
void check_port_1(uint8 Port1, uint8* p1_0, uint8* p1_1, uint8* in1)
{
    switch(Port1){
        case 1:{
            //W1_0
...
void Port2_Job(uint8 intr)
{
    ...
    check_port_2(Port2, &Port2_W0, &Port2_W1, &Port2_In);
...
void check_port_2(uint8 Port2, uint8* p2_0, uint8* p2_1, uint8* in2)
{
    switch(Port2){
        case 1:{
            //W2_0
...
CY_ISR(Port1_ISR)
{
	...
	intr1[intr1_counter] = ~Pins_Port1_Read() & 0b00000111;
	intr1_counter++;
...
CY_ISR(Port2_ISR)
{
	...
	intr2[intr2_counter] = ~Pins_Port2_Read() & 0b00000111;
	intr2_counter++;
...
void Timer_Intr1_Callback()
{
	...
	for(uint8_t i = 0; i < intr1_counter; i++)
		Port1_Job(intr1[i]);
	intr1_counter = 0;
	return;
}
...
void Timer_Intr2_Callback()
{
	...
	for(uint8_t i = 0; i < intr2_counter; i++)
		Port2_Job(intr2[i]);
	intr2_counter = 0;
	return;
}
...
void timers_create()
{
    ...
    OS_TIMER_Create(&Timer_Inps, Timer_Inps_Callback, 100);
    CyWdtClear();
